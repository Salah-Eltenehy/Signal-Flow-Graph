...
public class StrongConnectedComponents {
	...
	private ArrayList<ArrayList<Integer>>[] adjListOriginal = null;
	 ...
	 */
	public StrongConnectedComponents(ArrayList<ArrayList<Integer>>[] adjList) {
	...
	private void makeAdjListSubgraph(int node) {
		...
		for (int i = node; i < this.adjList.length; i++) {
			...
			for (int j = 0; j < this.adjListOriginal[i].size(); j++) {
				if (this.adjListOriginal[i].get(j).get(0) >= node) {
					successors.add(new Integer(this.adjListOriginal[i].get(j).get(0)));
				}
			}
